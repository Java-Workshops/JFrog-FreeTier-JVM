...
if [ "$JABBA_VERSION" == "latest" ]; then
    ...
    # should be redirected to own repo for DevSecOps/OnPrem env.
...
#    BINARY_URL=https://github.com/shyiko/jabba/releases/download/${JABBA_VERSION}/jabba-${JABBA_VERSION}-darwin-amd64
    BINARY_URL=https://svenruppert.jfrog.io/artifactory/generic-local-jabba/jabba-${JABBA_VERSION}-darwin-amd64
    ;;
    linux*)
    case "$(uname -m)" in
        arm*|aarch*)
...
#    BINARY_URL=https://github.com/shyiko/jabba/releases/download/${JABBA_VERSION}/jabba-${JABBA_VERSION}-linux-${OSARCH}
    BINARY_URL=https://svenruppert.jfrog.io/artifactory/generic-local-jabba/jabba-${JABBA_VERSION}-linux-${OSARCH}
    ;;
    *)
